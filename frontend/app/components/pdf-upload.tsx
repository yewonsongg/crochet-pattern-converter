  ...
  return (
    <div>
      ...
      {file ? (
        ...
        <div role="button" tabIndex={disabled ? -1 : 0} onClick={openFilePicker} onKeyDown={handleKeyDown} onDragEnter={(event) => { event.preventDefault(); if (!disabled) setIsDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setIsDragging(false); }} onDrop={handleDrop} aria-label="Upload crochet chart PDF" aria-disabled={disabled} className={`group cursor-pointer rounded-[1.4rem] border-2 border-dashed px-5 py-6 text-center transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4 ${isDragging ? "scale-[1.01] border-black bg-[#eceae2]" : "border-black/25 bg-white hover:border-black/60 hover:bg-[#fbfaf7]"} ${disabled ? "cursor-not-allowed opacity-50" : ""}`}>
          ...
          <p className="font-hand mt-3 text-lg font-bold">Drop your chart here</p>
