...
export default function Home() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#f7f6f2] text-[#171717]">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col px-5 py-6 sm:px-8 lg:px-12">
        <header className="flex items-center justify-between border-b border-black/12 pb-5">
          <a href="#top" className="group flex items-center gap-3 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4 focus-visible:ring-offset-[#f7f6f2]" aria-label="StitchScript home">
            <Image src="/cat-logo.svg" alt="" width={74} height={49} priority className="h-11 w-auto transition-transform group-hover:-rotate-2" />
            <span className="font-hand text-xl font-bold tracking-tight sm:text-2xl">StitchScript</span>
          </a>
        ...
        <section id="top" className="grid flex-1 items-center gap-10 py-14 lg:grid-cols-[0.88fr_1.12fr] lg:gap-16 lg:py-20">
          <div className="max-w-xl">
            <p className="mb-5 flex items-center gap-3 text-xs font-bold uppercase tracking-[0.2em] text-black/55"><span className="block h-px w-8 bg-black/40" />Chart to written pattern</p>
            <h1 className="font-hand text-[clamp(3.15rem,8vw,6.75rem)] font-bold leading-[0.88] tracking-[-0.055em]">
              Make every
            ...
            <p className="mt-10 max-w-lg text-base leading-7 text-black/65 sm:text-lg sm:leading-8">Upload a crochet chart PDF and preview how StitchScript will turn visual symbols into an editable, row-by-row pattern.</p>
        ...
        <footer className="flex flex-col gap-2 border-t border-black/12 py-5 text-xs text-black/50 sm:flex-row sm:items-center sm:justify-between">
