...
def generate_dtr(spec: ClassSpec, sample: SampledParameters, config: GenerationConfig) -> GeneratedObject:
  ...
  if bar_stem_ratio <= 0 or cross_bar_ratio <= 0 or stroke_width <= 0:
    raise ValueError("dtr ratios and stroke_width must be positive.")
  if not 0.0 <= cross_bar_y <= 1.0:
    raise ValueError("dtr cross_bar_y must be in the range [0, 1].")
  if not -45.0 <= cross_bar_angle_deg <= 45.0:
    raise ValueError("dtr cross_bar_angle_deg must be in the range [-45, 45].")
  ...
  geometry = build_bar_stem_svg(
    ...
    cross_bar_count=3, stroke_width=stroke_width,
  )
  metadata = {
...
def _finite_number(value: Any, name: str) -> float:
  if isinstance(value, bool) or not isinstance(value, (int, float)):
    raise ValueError(f"dtr {name} must be a finite number.")
  result = float(value)
